...
{
    private function processNHL($oNode)
    {
    ...
    public function parseXML($a_sXML)
    {
        ...
        foreach ($oXML->NewDataSet->GameLines as $cEvent)
        {
            ...
            if ((trim((string) $cEvent->SportType) == 'Hockey'
                    && (trim((string) $cEvent->SportSubType) != 'NHL')
                    && ((int) $cEvent->IsCancelled) != 1
                    && ((int) $cEvent->isGraded) != 1)
            )
            {
                $this->processNHL($cEvent, $oSport);
            }



                $oParsedSport = new ParsedSport('NHL');
