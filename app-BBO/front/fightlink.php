...
class FightLinkCreator
{
    ...
    public static function createLink($aFights, $a_iLineType, $a_iFormat) //1 = Moneyline, 2 = Decimal
    {
        ...
        for ($iFightX = 0; $iFightX < count($aFights); $iFightX++)
        {
            ...
            self::textCustomSpacing($rImage, FONT_SIZE, 0, 6, 0 + LINK_BFO_HEIGHT + $iFighterCellHeight + ($iFighterCellHeight * ($iFightX * 2)) + $iFightX - 5, $rTextColor, FONT_TYPE, $oFight->getFighterAsString(1), 2);
            ...
            //Fighter 2 Name
            self::textCustomSpacing($rImage, FONT_SIZE, 0, 6, 0 + LINK_BFO_HEIGHT + ($iFighterCellHeight * 2) + ($iFighterCellHeight * ($iFightX * 2)) + $iFightX - 6, $rTextColor, FONT_TYPE, $oFight->getFighterAsString(2), 2);
