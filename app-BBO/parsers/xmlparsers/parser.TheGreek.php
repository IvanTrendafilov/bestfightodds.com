...
class XMLParserTheGreek
{
    ...
    private function collectMMAEvents($a_sPage)
    {
        ...
        $sEventRegexp = '<a href="(\/sports\/contest\/contestdisplay\\.asp\\?Sport=Boxing&ContestType=Boxing&ContestType2=.*?((MMA)|(UFC)|(Ultimate%20Fighter)).*?)">';
        ...
        foreach ($aURLList as $sURL)
        {
            ...
            $sFightRegexp = '/\\s[0-9]{4}\\s([a-zA-Z�]+[a-zA-Z�0-9"\\s\\-.,]+?)\\s*([+-]{0,1}[0-9]+|EV|even)\\s+[0-9]+\\s([a-zA-Z�]+[a-zA-Z�0-9"\\s\\-.,]+?)\\s*([+-]{0,1}[0-9]+|EV|even)/';
            ...
            {
                $sRetXML .= '<fight><f1>' . $aFight[1] . '</f1><f2>' . $aFight[3] . '</f2><f1_line>' . $aFight[2] . '</f1_line><f2_line>' . $aFight[4] . '</f2_line></fight>';
            }
        }
