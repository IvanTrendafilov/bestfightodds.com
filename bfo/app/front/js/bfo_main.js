...
initPage = function () {
    ...
    document.querySelectorAll('.odds-table').forEach(function (item) {
        item.addEventListener("click", function (e) {
            e.stopPropagation();
            //Traverse upwards and find the cell
            var td = e.target;
            ...
            else if (td.matches('td.but-sgp')) //Show graph for prop odds
            {
                if (parlayMode) {
                    return addToParlay(td);
                } else {
            ...
            else if (td.matches('td.but-sgep')) //Show graph for event prop odds
            {
                if (parlayMode) {
                    return addToParlay(td);
                } else {
                ...
                if (parlayMode) {
                    ...
                    var opts = JSON.parse(td.dataset.li);
                    var title = td.parentNode.querySelector('th').innerText + " <span style=\"font-weight: normal;\"> &#150; Mean odds";
                    chartCC();
                ...
                context.teamtitle = $(td).closest("tr").find("th").text();
                if (parlayMode) {
                    return addToParlay(td);
                } else {
                    alertSW(context, e.clientX, e.clientY);
                    return false;
                }
            }
                ...
                $(td).data('toggled', !$(td).data('toggled'));
                $("[data-mu=" + matchup_id + "]").data('toggled', $(td).data('toggled'));
                if ($(td).data('toggled')) {
                    if (navigator.appName.indexOf("Microsoft") > -1 && navigator.appVersion.indexOf("MSIE 10.0") == -1) {
                        $(td).closest('tr:not(.pr)').next('tr:not(.pr)').addBack('tr').nextUntil('tr:not(.pr)').css('display', 'block');
                        $('#mu-' + matchup_id).next('tr:not(.pr)').nextUntil('tr:not(.pr)').css('display', 'block');
                    } else {
                        $(td).closest('tr:not(.pr)').next('tr:not(.pr)').addBack('tr').nextUntil('tr:not(.pr)').css('display', 'table-row');
                    ...
                    $(td).closest('tr:not(.pr)').next('tr').addBack('tr').nextUntil('tr:not(.pr)').css('display', 'none');
