...
/**
 * The alerter is a feature that warns (e-mails) a user whenever odds for a certain fight drops below a certain number
 */
class Alerter
{
    public static function addNewAlert(int $matchup_id, int $team_no, string $email, string $odds_limit, int $bookie_id, int $odds_type = 1): int
    {
    ...
    public static function dispatchAlert(Alert $alert_obj): bool
    {
        ...
        if ($alert_obj->getLimit() == -9999) {
            $sText = "Odds for " . $matchup->getTeamAsString(1) . " (" . $team_odds[1] . ") vs " . $matchup->getTeamAsString(2) . " (" . $team_odds[2] . ") has just been posted at " . ALERTER_SITE_NAME . "\n
...
" . ALERTER_SITE_NAME;
            ...
            $sMessageHTML = "<b>Alert: New odds added</b><br><br>" . $matchup->getTeamAsString(1) . " <b>" . $team_odds[1] . "</b><br>" . $matchup->getTeamAsString(2) . " <b>" . $team_odds[2] . "</b><br>";
            $sSubject = 'Odds for ' . $matchup->getTeamAsString(1) . ' vs ' . $matchup->getTeamAsString(2) . ' available';
        } else {
            $sText = "The odds for " . $matchup->getTeamAsString($alert_obj->getFighter()) . " has reached " . $team_odds[$alert_obj->getFighter()] . " in his/her upcoming fight against " . $matchup->getTeamAsString(($alert_obj->getFighter() == 1 ? 2 : 1)) . "\n
Visit " . ALERTER_SITE_LINK . " to view the latest odds listings.\n
...
" . ALERTER_SITE_NAME;
            ...
            $sMessageHTML = "<b>Alert: Odds changed</b><br><br>The odds for " . $matchup->getTeamAsString($alert_obj->getFighter()) . " has reached " . $team_odds[$alert_obj->getFighter()] . " in his/her upcoming fight against " . $matchup->getTeamAsString(($alert_obj->getFighter() == 1 ? 2 : 1)) . "<br>";
            $sSubject = 'Odds for ' . $matchup->getTeamAsString($alert_obj->getFighter()) . ' has reached your limit';
        }
