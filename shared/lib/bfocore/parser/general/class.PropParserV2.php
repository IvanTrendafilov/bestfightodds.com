...
class PropParserV2
{
    ...
    public function matchProps($props)
    {
        ...
        {
            $result[] = $this->matchSingleProp($prop);
        }
        return $result;
    }
    ...
    public function matchPropToTemplate($prop, $bookie_id)
    {
        ...
        foreach ($templates as $template)
        {
            ...
            $aPVMatches = array();
            $bFound1 = (preg_match('/^' . $template_str . '$/', $a_oProp->getTeamName(1), $aPVMatches1) > 0);
            $bFound2 = (preg_match('/^' . $template_str . '$/', $a_oProp->getTeamName(2), $aPVMatches2) > 0);
            if ($bFound1 && $bFound2)
            {
                $this->logger->warning('--Both team fields match template. Picking shortest one as it is probably not the negative one: ' .  $a_oProp->getTeamName(1) . ' compared to ' .  $a_oProp->getTeamName(2));
    ...
    {
        $iCounter = 0;
        $this->logger->log('Starting parsing of props', 2);

        foreach ($a_aProps as $oProp)
        {
            $this->logger->log('-Parsing: ' . $oProp->toString(), 2);
    ...
    public function parseSingleProp($a_iBookieID, &$a_oProp)
    {
        ...
        if ($oTemplate == null)
        {
            $this->logger->log('--No template found for ' . $a_oProp->toString() . 
        ...
        $this->logger->log('--Template found for ' . $a_oProp->toString() . ': ' . $oTemplate->toString(), 1);
        ...
        else
        {
            ...
            {
                $this->logger->log('--Matchup has correlation ID from bookie but no match. Checking all matchups', 0);
            ...
            {
                $this->logger->log('---No matchup found for prop values ' . $a_oProp->toString() . ' (Template ' . $oTemplate->getID() . ' expecting ft: ' . $oTemplate->getFieldsTypeAsExample() . ')' .
            ...
            $this->logger->log('---We found a match for prop values! (' . $aResult['matchup'] . ')', 2);
            ...
            {
            	$this->logger->log('----Bookie does not have normal odds for matchup, bailing', -1);
            	return false;
            }
            ...
            {
                $this->logger->log("------- nothing has changed since last prop odds", 2);
                return true;
            }
            else
            {
                $this->logger->log("------- adding new prop odds!", 2);
    ...
    public function matchParsedPropToTemplate($a_iBookieID, &$a_oProp)
    {
        ...
        foreach ($aTemplates as $oTemplate)
        {
            ...
            if ($bFound1 && $bFound2)
            {
                $this->logger->log('--Both team fields match template. Picking shortest one as it is probably not the negative one: ' .  $a_oProp->getTeamName(1) . ' compared to ' .  $a_oProp->getTeamName(2), 0);
                //TEMPORARY HACK FOR PROPTYPE 65. REMOVE WHEN NO ODDS ARE LIVE FOR THIS TYPE
                if ($oTemplate->getPropTypeID() == 65)
                {
                    $this->logger->log('---Proptype 65, picking main prop 1 anyway. TODO: Remove this when no odds are live!', -1);
                    $a_oProp->setMainProp(1);
                    $aPVMatches = $aPVMatches1;
                }
                else if (strlen($a_oProp->getTeamName(1)) <= strlen($a_oProp->getTeamName(2)))
                {
                    $this->logger->log('---Field 1 wins', 0);
                    $a_oProp->setMainProp(1);
                    $aPVMatches = $aPVMatches1;
                }
                else
                {
                    $this->logger->log('---Field 2 wins', 0);
                    $a_oProp->setMainProp(2);
                    $aPVMatches = $aPVMatches2;
                }
            }
            ...
            if ($bFound == true)
            {
                ...
                if ($oFoundTemplate != null)
                {
                    $this->logger->log('--Warning: Multiple templates matched. Will accept longest one', 0);
                }
                ...
                if ($oFoundTemplate == null || strlen($oTemplate->getTemplate()) > strlen($oFoundTemplate->getTemplate()))
                {
                    ...
                    if (count($aPVMatches) == 0)
                    {
                        $this->logger->log('--No prop values fetched, make sure that no invalid prop templates exist');
                    }
                    else
                    {
                        //Store the prop values in the prop
    ...
    public function matchParsedPropToMatchup($a_oProp, $a_oTemplate)
    {
        ...
        $this->logger->log('--- searching for manual correlation: ' . $sSearchProp . ' for bookie ' . $a_oTemplate->getBookieID(), 1);
        $sFoundCorrMatchup = OddsHandler::getMatchupForCorrelation($a_oTemplate->getBookieID(), $sSearchProp);
        if ($sFoundCorrMatchup != null)
        {
            $this->logger->log('---- prop has manual correlation stored: ' . $sFoundCorrMatchup, 1);
            $oPreMatchedMatchup = EventHandler::getFightByID($sFoundCorrMatchup);
            if ($oPreMatchedMatchup != null)
            {
                $this->logger->log('----- found stored manual correlation for ' . $sSearchProp . ' : ' . $oPreMatchedMatchup->getID(), 2);
        ...
        {
            $this->logger->log('---- no stored manual correlation found', 1);
            ...
            {
                $this->logger->log('--- prop has correlation id: ' . $a_oProp->getCorrelationID(), 1);
                $oPreMatchedMatchup = EventHandler::getFightByID(ParseTools::getCorrelation($a_oProp->getCorrelationID()));
                if ($oPreMatchedMatchup != null)
                {
                    $this->logger->log('--- found stored correlation: ' . $oPreMatchedMatchup->getID(), 2);
                    $aMatchupsToCheck = array($oPreMatchedMatchup);
                    //Even though we have prematched the matchup, we still need to add alt name matchups
                    $aMatchupsToCheck = array_merge($aMatchupsToCheck, $this->addAltNameMatchupsToMatchup($oPreMatchedMatchup));
                }
                ...
                {
                    $this->logger->log('--- no stored correlation found (' . $a_oProp->getCorrelationID() . ')', 1);
                }
            }
        }
            ...
            foreach ($aMatchupsToCheck as $oMatchup)
            {
                ...
                switch ($a_oTemplate->getFieldsTypeID())
                {
                    ...
                    case 8:
                        ...
                        break;
                    default:
                        $this->logger->log('---Unknown fields type ID in PropTemplate: ' . $a_oTemplate->getFieldsTypeID(), -2);
                        return null;
                }
                ...
                for ($iX = 0; $iX < count($aParsedMatchup); $iX++)
                {
                    if (($bFound == false && $iX == 0) || ($bFound == true && $iX > 0))
                    {
                        ...
                        //DEBUG:
                        /*$this->logger->log("checking: " . $aStoredMatchup[$iX] . " vs " . $aParsedMatchup[$iX] . " fsim:" . $fSim, -2);
                ...
                if ($bFound == true)
                {
                    ...
                    {
                        $this->logger->log('---Found multiple matches for prop values. Comparing fsims, challenger: ' . $oMatchup->getTeamAsString(1) . ' vs ' . $oMatchup->getTeamAsString(2) . ' ' . $oMatchup->getID() . ' (' . $fNewSim . ') and current: ' . $oFoundMatchup->getTeamAsString(1) . ' vs ' . $oFoundMatchup->getTeamAsString(2) . ' ' . $oFoundMatchup->getID() . ' (' . $fFoundSim . ')', 0);
                        if ($fNewSim > $fFoundSim)
                        {
                            $oFoundMatchup = $oMatchup;
                            $iFoundMatchupID = $oFoundMatchup->getID();
                            $fFoundSim = $fNewSim;
                            $this->logger->log('----Challenger won, changing matched to new one: ' . $iFoundMatchupID);
    ...
    private function matchParsedPropToEvent($a_oProp, $a_oTemplate)
    {
        ...
        foreach ($aEventsToCheck as $oEvent)
        {
            ...
            //DEBUG:
            /*$this->logger->log("checking: " . $oEvent->getName() . " _and " . substr(strtoupper($oEvent->getName()), 0, strpos($oEvent->getName(), ':')) . " vs " . $aParsedEvent[0] . " fsim:" . $fSim, -2);
