...
<div id="page-wrapper">
    ...
    <div id="page-container">
        <div id="page-inner-wrapper">
            <div id="page-content">
            <?php
            $aSwings = StatsHandler::getAllDiffsForEvent($_GET['eventID']);
            //Get total diff
            $fTotSwing = $aSwings[0][2]['swing'] - $aSwings[count($aSwings) - 1][2]['swing'];
            foreach ($aSwings as $aSwing)
            {
                /*if ($aSwing[2]['swing'] > 0)
                {*/
                    echo '<p style="float: left; font-size: 10px;">'  . $aSwing[0]->getTeamAsString($aSwing[1]) . '</p><br />';
                    echo '<div style="background-color: #ff0000; float:left; height: 10px; font-size: 10px; width: '  . round(($aSwing[2]['swing'] / $aSwings[0][2]['swing']) * 100) . 'px;"></div> ' . $aSwing[2]['opening'] . ' --> ' . $aSwing[2]['latest']  . ' (' . $aSwing[2]['swing'] . ')';
