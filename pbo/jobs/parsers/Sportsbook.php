 ...
 * Props: No
 *
 * URL: https://www.gamingsystem.ag/sbk/sportsbook4/www.sportsbook.ag/getodds.xgi?categoryId=104
 *
 * Timezone in feed: ET
 * 
 */
...
class ParserJob
{
    ...
    public function run($mode = 'normal')
    {
        ...
        {
            //1. Boxing
            $urls = ['https://www.gamingsystem.ag/sbk/sportsbook4/www.sportsbook.ag/getodds.xgi?categoryId=104'];
    ...
    public function parseContent($content)
    {
        ...
        if (!$failed_once && count($parsed_sport->getParsedMatchups()) >= 3)
        {
            $this->full_run = true;
    ...
    public function parseContentNew($content)
    {
        ...
        //Declare authorative run if we fill the criteria
        if (!$failed_once && count($parsed_sport->getParsedMatchups()) >= 1)
        {
            $this->full_run = true;
            $this->logger->info("Declared full run");
        }
