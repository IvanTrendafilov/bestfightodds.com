...
class ParserJob
{
    ...
    public function run($mode = 'normal')
    {
        ...
        if ($mode == 'mock') {
            ...
            $matchups_url = 'https://feeds.betway.com/sbeventsen?key=1E557772&keywords=boxing';
            ...
            if ((string) $cEvent['started'] != 'true') //Disable live odds
            {
                ...
                if ($event_name != 'Submission Underground' && $event_name != 'BKFC') {
                    foreach ($cEvent->Markets->Market as $cMarket) {
                        if (((string) $cMarket['cname'] == 'fight-winner' || (string) $cMarket['cname'] == 'fight-winner-') && count($cMarket->Outcomes->Outcome) == 3) {
                            //Regular matchup
                            $oParsedMatchup = new ParsedMatchup(
                                (string) $cMarket->Outcomes->Outcome[0]->Names->Name,
                                (string) $cMarket->Outcomes->Outcome[2]->Names->Name,
                                OddsTools::convertDecimalToMoneyline((float) $cMarket->Outcomes->Outcome[0]['price_dec']),
                                OddsTools::convertDecimalToMoneyline((float) $cMarket->Outcomes->Outcome[2]['price_dec'])
                            );
                            ...
                            (string) $cMarket['cname'] == 'knocked-down' ||
                            (string) $cMarket['cname'] == 'total-knockdowns' || 
                            ...
                            (string) $cMarket['cname'] == 'round-group-betting' ||
                            (string) $cMarket['cname'] == 'fight-specials' ||
                            (string) $cMarket['cname'] == 'knocked-down---win'
                        ) {
                            //Single line prop
