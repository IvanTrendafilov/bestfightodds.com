    ...
    if ($bCached == false || empty($sBuffer))
    {
        ...
        foreach ($aFights as $oFight)
        {
            ...
            for ($iX = 1; $iX <= 2; $iX++)
            {
                ...
                echo '<td class="date-cell">' . ($iX == 1 ? $sGameTime : ($sGameTime != '' ? 'UTC' : '')) . '</td><th scope="row"><a href="/fighters/' . $oFight->getFighterAsLinkString($iX) . '"><span class="t-b-fcc">' . $oFight->getFighterAsString($iX) . '</span></a></th>';
                echo '</tr>';
            }
            ...
            if (count($aPropTypes) > 0)
            {
                ...
                foreach ($aPropTypes as $oPropType)
                {
                    ...
                    for ($iX = 1; $iX <= 2; $iX++)
                    {
                        ...
                        $oPropType->setPropDesc(str_replace('<T2>', $oFight->getTeamLastNameAsString(($oPropType->getTeamNum() % 2) + 1), $oPropType->getPropDesc()));
       ...
       if (!$bSomeOddsFound)
        {
            ...
            {
                echo '<br /><br /><img src="/img/info.gif" class="img-info-box" /> Get notified when odds are posted either via our <a href="/alerts" style="text-decoration: underline">Alerts</a> functionality or by following us on <a href="http://twitter.com/proboxingodds" target="_blank" rel="noopener" style="text-decoration: underline">Twitter</a>';
            }
            echo '</td></tr>';
        }
